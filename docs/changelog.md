...
!!! Abstract "新增功能 :star2:"
    - feat(数据源): 支持 Impala 数据源
    - feat(视图): 增加视图类文本卡
    - feat(视图): 视图跳转支持点击值作为参数
    - feat(仪表板): 支持网页组件
    - feat(仪表板): 公共链接支持第三方系统参数
    - feat(仪表板): Tab 组件支持内部样式编辑
    - feat(仪表板): 具有移动端的仪表板增加标识，用以快速区分该仪表板是否存在移动端布局
    - feat(仪表板): 新建仪表板支持导入模板的方式
    - feat(仪表板): 仪表板模板创建的仪表板支持样式、模板数据的迁移
    - feat(XPack): 行权限支持内置变量
    - feat(XPack): 新增在【显示设置】中配置移动端登录页面背景
    - feat(XPack): 分组权限自动继承
    - feat(安装部署): 支持多种部署方式
    - feat(安装部署): 支持高可用模式，可在引擎设置处分别配置 Doris 与 Kettle 信息
    - feat(安装部署): 支持插件管理
    - feat: 支持 Excel、API 数据保存在系统引擎

!!! Abstract "功能优化 :sunflower:"
    - refactor(视图): 支持辅助线设置
    - refactor(视图): 支持缩略轴设置
    - refactor(视图): 仪表盘分段显示颜色
    - refactor(视图): 透视表支持小计合计
    - refactor(仪表板): 支持在仪表板编辑视图
    - refactor(仪表板): 组件背景统一到仪表板设置
    - refactor(系统管理): 是否显示首页转到基础设置界面
